...
class PPOAgent(PGAgent):
    ...
    def save_model(self, out_path):
        ...
        torch.save(checkpoint, out_path+'_ppo_agent')

    def load_model(self, in_path):
        # super().save_model(in_path)
        checkpoint = torch.load(in_path+"_ppo_agent")
    ...
    def _train_step(self):
        ...
        global_sample_count = int(local_sample_count)# int(MPIUtil.reduce_sum(local_sample_count))
        ...
        for e in range(self.epochs):
            ...
            for b in range(mini_batches):
                ...
                actor_loss += np.abs(curr_actor_loss.detach().numpy())
        ...
        critic_stepsize = "i dunno"# self._critic_solver.get_stepsize()
        actor_stepsize = "i dunno"# self._actor_solver.get_stepsize()

        print("###################################")
        print('Critic_Loss', critic_loss)
        print('Critic_Stepsize', critic_stepsize)
        print('Actor_Loss', actor_loss) 
        print('Actor_Stepsize', actor_stepsize)
        print('Clip_Frac', actor_clip_frac)
    ...
    def _update_critic(self, s, g, tar_vals):
        ...
        # print('critic',self._critic_tf(critic_input)[:10], critic_input[:10], tar_vals[:10])
        val_diff = torch.FloatTensor(tar_vals) - self._critic_tf(critic_input)
        critic_loss_tf = 0.5 * torch.mean(torch.square(val_diff))
        self._critic_opt.zero_grad()
        critic_loss_tf.backward()
        self._critic_opt.step()

        # print("update critic")
        # print("critic_input", critic_input)
        # print("val_diff", val_diff)
        # print("tar_vals", tar_vals)
        print("loss", critic_loss_tf)

        return critic_loss_tf
    
    def _update_actor(self, s, g, a, logp, adv):
        actor_input = self._get_actor_inputs(s, g)

        norm_tar_a_tf = self._a_norm.normalize_tf(a)
        mean_actor = self._norm_a_pd_tf.mean(actor_input)
        _a_logp_tf = self._norm_a_pd_tf.logp(norm_tar_a_tf, mean_actor)
        logp = torch.FloatTensor(logp)
        ratio_tf = torch.exp(_a_logp_tf - logp)
        adv = torch.FloatTensor(adv)
        actor_loss0 = adv * ratio_tf
        actor_loss1 = adv * torch.clip(ratio_tf, 1.0-self.ratio_clip, 1.0+self.ratio_clip)
        actor_loss_tf = torch.minimum(actor_loss0, actor_loss1)
        loss = -torch.mean(actor_loss_tf)
        ...
        clip_frac = torch.mean(torch.greater(torch.abs(ratio_tf-1.0), self.ratio_clip).float())
