...
class RLAgent(ABC):
    ...
    def __init__(self, json_data, env, writer: SummaryWriter):
        self.env = env
        self.writer = writer
        ...
        if (self.path.pathlength() > 0):
            ...
            if (self._mode == self.Mode.TRAIN):
                if (self.enable_training and self.path.pathlength() > 0):
                    # print(self.path.pathlength(), len(self.path.states), len(self.path.goals), len(self.path.actions), len(self.path.logps), len(self.path.rewards), len(self.path.flags))
                ...
                assert False, print("Unsupported RL agent mode" + str(self._mode))
    ...
    def get_action_space(self):
        return 1#self.world.env.get_action_space(self.id)

    def get_state_size(self):
        return self.env.GetStateDim()# self.world.env.get_state_size(self.id)

    def get_goal_size(self):
        return 0#self.world.env.get_goal_size(self.id)

    def get_action_size(self):
        return self.env.GetActionDim()# self.world.env.get_action_size(self.id)

    def get_num_actions(self):
        return 0#self.world.env.get_num_actions(self.id)
    
    def need_new_action(self):
        return True#self.world.env.need_new_action(self.id)

    def _build_normalizers(self):
        self._s_norm = Normalizer(self.get_state_size(), self.env.build_state_norm_groups(self.id))
        self._s_norm.set_mean_std(-self.env.build_state_offset(self.id), 
                                 1 / self.env.build_state_scale(self.id))

        self._g_norm = Normalizer(self.get_goal_size(), self.env.build_goal_norm_groups(self.id))
        self._g_norm.set_mean_std(-self.env.build_goal_offset(self.id), 
                                 1 / self.env.build_goal_scale(self.id))

        self._a_norm = Normalizer(self.get_action_size())
        self._a_norm.set_mean_std(-self.env.build_action_offset(self.id), 
                                 1 / self.env.build_action_scale(self.id))
        return

    def _build_bounds(self):
        self._a_bound_min = self.env.build_action_bound_min(0)
        self._a_bound_max = self.env.build_action_bound_max(0)
        return
    ...
    def _record_state(self):
        s = self.env.RecordAgentObs()
        return s

    def _record_goal(self):
        g = 0#self.world.env.record_goal(self.id)
        return g

    def _record_reward(self):
        r = 0#TODO: return 0 first #self.world.env.calc_reward(self.id)
        return r
    ...
    def _end_path(self):
        ...
        # print("endpathbefore",self.path.pathlength(), len(self.path.states), len(self.path.goals), len(self.path.actions), len(self.path.logps), len(self.path.rewards), len(self.path.flags))

        self.path.rewards.append(r)
        self.path.states.append(s)
        self.path.goals.append(g)
        self.path.terminate = self.env.terminate()
        # print("endpathafter",self.path.pathlength(), len(self.path.states), len(self.path.goals), len(self.path.actions), len(self.path.logps), len(self.path.rewards), len(self.path.flags))
    ...
    def _update_new_action(self):
        ...
        self.path.states.append(s)
        self.path.goals.append(g)
        self.path.actions.append(a.detach().numpy())
        self.path.logps.append(np.array([logp.detach().numpy()]))
    ...
    def _update_test_return(self, path):
        ...
        if (self.test_episode_count * 1 >= self.test_episodes):#MPIUtil.get_num_procs()
            global_return = self.test_return#MPIUtil.reduce_sum(self.test_return)
            global_count = self.test_episode_count#MPIUtil.reduce_sum(self.test_episode_count)
    ...
    def _init_mode_train(self):
        self._mode = self.Mode.TRAIN
        # self.world.env.set_mode(self._mode)   #TODO: don't have this mode yet
        return
    ...
    def _enable_output(self):
        return True#MPIUtil.is_root_proc() and self.output_dir != ""

    def _enable_int_output(self):
        return True#MPIUtil.is_root_proc() and self.int_output_dir != ""
    
    def _calc_val_bounds(self, discount):
        r_min = 0#-np.inf#TODO: try inf here first #self.world.env.get_reward_min(self.id)
        r_max = 1#np.inf# #self.world.env.get_reward_max(self.id)
        assert(r_min <= r_max)
    ...
    def _calc_term_vals(self, discount):
        r_fail = 0#TODO: try inf here first #self.world.env.get_reward_min(self.id)
        r_succ = 1# self.world.env.get_reward_succ(self.id)

        r_min = 0#-np.inf#self.world.env.get_reward_min(self.id)
        r_max = 1#np.inf#self.world.env.get_reward_max(self.id)
    ...
    def _enable_draw(self):
        return self.env.draw# self.world.env.enable_draw
    ...
    def _store_path(self, path: Path):
        path_id = self.replay_buffer.store(path)
        valid_path = path_id != -1#MathUtil.INVALID_IDX
    ...
    def _record_normalizers(self, path: Path):
    ...
    def _train(self):
        ...
        self._total_sample_count = int(samples)#MPIUtil.reduce_sum(samples))
        end_training = False
        ...
        if (self.replay_buffer_initialized):  
            if (self._valid_train_step()):
                ...
                avg_train_return = self.train_return#MPIUtil.reduce_avg(self.train_return)
                ...
                for i in range(iters):
                    ...
                    print("Samples", self._total_sample_count)
                    print("Train_Return", avg_train_return)
                    print("Test_Return", self.avg_test_return)
                    print("State_Mean", s_mean)
                    print("State_Std", s_std)
                    print("Goal_Mean", g_mean)
                    print("Goal_Std", g_std)
                    print("####################")
                    print("")
            ...
            print("Samples: " + str(self._total_sample_count))
            print("") 
    ...
    def _get_iters_per_update(self):
        return 1 * self.iters_per_update#MPIUtil.get_num_procs()
    ...
    def _log_exp_params(self):
        print("Exp_Rate", self.exp_params_curr.rate)
        print("Exp_Noise", self.exp_params_curr.noise)
        return